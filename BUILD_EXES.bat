...
REM Build each tool
echo Building Compiler.exe...
pyinstaller --onefile --name Compiler --distpath ..\bin Compiler.py > nul 2>&1

echo Building ImageConverter.exe...
pyinstaller --onefile --name ImageConverter --distpath ..\bin ImageConverter.py > nul 2>&1

echo Building Linker.exe...
pyinstaller --onefile --name Linker --distpath ..\bin Linker.py > nul 2>&1
